...
template <class T>
Node<T>* GraphEdgeList<T>::nodeExistence(const T& value){
    Node<T>* node = nullptr;
...
template<class T>
TreePtrList<T>& GraphEdgeList<T>::breadthSearch(const T& startValue, TreePtrList<T>& tree){
	...
	*/
	...
	// for(auto &e: this->edgeList){
    //     e.src.mark = unexplored;
    //     e.dest.mark = unexplored;
    // }
		
	//ricerca del nodo da cui partire
	// typename std::vector<Edge<T>>::iterator edgeItr;
	// edgeItr = this->edgeList.find(startValue);
	// if(edgeItr == incList.end()){
	...
	// _tree.addNode(new)
    // if(!this->nodeExistence(startValue)){
...
template<class T>
TreePtrList<T>& GraphEdgeList<T>::depthSearch(const T &_first_node, TreePtrList<T> &_tree) {
